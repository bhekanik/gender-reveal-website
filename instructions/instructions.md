...
- **Purpose:** Give a way to add and update the settings of the site.
- **Details:**
  - **Account Name:** An account name that will be turned into kebab case and used as the slug for the baby site. Only characters that can be used in a URL are allowed.
  - **Announcement Date:** Users can set the date and time of when they intend to reveal the information about the baby's gender.
  - **Babies:** A control that allows users to add the babies they're having and their genders, ensuring support for people with one, two or more babies. The control will allow adding of each baby and set the baby's gender. It also allows all the usual CRUD operations.
  - **Features:** A section where a user can select what features they want in their site. By features I mean, they can select whether to show the countdown, whether to show the baby gender poll and whether to show the baby gender quiz.
  - **CMS:** A CMS section that will give the user a place to update the copy that is shown in the welcome hero, and in the reveal page.
  - **Danger Zone:** A section that will allow the user to delete their account and all it's information.
